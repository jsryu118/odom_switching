...
class KF {
   public:
    ...
    explicit KF(double pos_n = 1e-2, double vel_n = 1e-4)
        : pos_noise_(pos_n),
          vel_noise_(vel_n) {
        // : acc_noise_(acc_n),
        //   gyr_noise_(gyr_n),
        ...
        state_ptr_->cov.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity() * pos_noise_;
        state_ptr_->cov.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() * vel_noise_;
     ...
     * @param curr_imu 
     */
    void predict(const Eigen::Matrix<double, kStateDim, kStateDim>& F, const Eigen::Matrix<double, kStateDim, kStateDim>& Q) {
        state_ptr_->x = F * state_ptr_->x;
        state_ptr_->cov = F * state_ptr_->cov * F.transpose() + Q;
        // std::cout << state_ptr_->cov << std::endl;
    }
     ...
     * @param r 
     */
    template <class H_type, class R_type, class Z_type>
    void update(const Eigen::MatrixBase<H_type>& H, const Eigen::MatrixBase<R_type>& R, const Eigen::MatrixBase<Z_type>& z) {
        EIGEN_STATIC_ASSERT_FIXED_SIZE(H_type);
        EIGEN_STATIC_ASSERT_FIXED_SIZE(R_type);
        EIGEN_STATIC_ASSERT_FIXED_SIZE(Z_type);

        const auto& P = state_ptr_->cov;
        const auto& x = state_ptr_->x;
        const auto S = H * P * H.transpose() + R;
        const auto K = P * H.transpose() * S.inverse();

        const Eigen::Matrix<double, kStateDim, 1> y = z - H * x;
        state_ptr_->x += K * y;
